...
public class mMarca_Producto extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        lblTitulo.setText("Mantenimiento Marca");
            ...
            if(this.txtNombre.getText().length() > 0)
            {
                if(this.txtDescripcion.getText().length() > 0)
                {
                    if(int_Modo == 1)
                    {
                        ...
                        con = new Conectar();  
                        ps = con.getConnection().prepareStatement("SELECT prod_brand_name FROM product_brand WHERE prod_brand_name = ?");
                        ps.setString(1, this.txtNombre.getText());
                        rs = ps.executeQuery();
                        con.Desconectar();
                        if(rs.next() == true)
                        {
                            JOptionPane.showMessageDialog(null, "La marca del producto fue registrado anteriormente", "Marca del producto registrada", JOptionPane.ERROR_MESSAGE);
                        }else
                        {
                ...
                {
                    JOptionPane.showMessageDialog(null, "Debe digitar la descripción de la marca del producto", "Error en validación", JOptionPane.ERROR_MESSAGE);
                }   
            }
            else
            {
                JOptionPane.showMessageDialog(null, "Debe digitar el nombre de la marca del producto", "Error en validación", JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception e) {
    ...
    void cargar_datos()
    {
        try {
            con = new Conectar();
            ps = con.getConnection().prepareStatement("SELECT * FROM product_brand");
            rs = ps.executeQuery();
            String[] columnas = {"ID", "Marca", "Usuario", "Creado", "Modificado"};
            ...
            while(rs.next())
            {
                int_Codigo = rs.getInt("prod_brand_id");
                str_Nombre = rs.getString("prod_brand_name");
                str_Usuario = rs.getString("user_name");
                date_FechaCreate = Formato.format(rs.getDate("prod_brand_create_date"));
                date_FechaModify = Formato.format(rs.getDate("prod_brand_modify_date"));
    ...
    private void Agregar()
    {
        try {
                ...
                int_Res = cls_Product.Insertar_Marca(str_Nombre, str_Descripcion ,str_Usuario, formato.format(date_FechaCrear), formato.format(date_FechaModificar));
                if(int_Res > 0)
                {
    ...
    {
        try {
            rs = cls_Product.Buscar_Marca(int_Code);
            if(rs.next())
            {
                str_Nombre = rs.getString("prod_brand_name");
                str_Descripcion = rs.getString("prod_brand_desc");
                str_Usuario = rs.getString("user_name");
                date_FechaCrear = rs.getDate("prod_brand_create_date");
                date_FechaModificar = rs.getDate("prod_brand_create_date");
    ...
    {
         try {
             int_Res = cls_Product.Borrar_Marca(int_Codigo);
             if(int_Res > 0)
             {
    ...
    private void Modificar()
    {
         try {
                ...
                date_FechaModificar = new Date();
                DateFormat formato = new SimpleDateFormat("YYYY-MM-dd");
                int_Res = cls_Product.Editar_Marca(str_Nombre, str_Descripcion ,str_Usuario, formato.format(date_FechaCrear), formato.format(date_FechaModificar), int_Codigo);
                if(int_Res > 0)
                {
