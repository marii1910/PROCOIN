...
public class mProveedor extends javax.swing.JFrame {
    //Declaracion de variables
    Conectar con;
    PreparedStatement ps;
    ResultSet rs;
    String str_Boton = "Aceptar", str_Codigo = null, str_Cedula = null, str_Nombre = null, str_Pais = null, 
            str_Ciudad = null, str_Calle = null, str_CodigoPostal = null, str_Telefono = null, str_Contacto = null, str_Correo = null,
    ...
    clsProveedor cls_Supplier = new clsProveedor();
    ...
    public mProveedor() {
        ...
        Integer [] int_cmbPlazoCredito = {15,30,45,60,90};
                                                        ...
                                                        if(this.cmbTipoMoneda.getSelectedIndex() > -1)
                                                        {
                                                            if(this.cmbTipoPago.getSelectedIndex() > -1)
                                                            {
                                                                if(this.cmbTipoPlazo.getSelectedIndex() > -1)
                                                                {
                                                                    ...
                                                                    {
                                                                        JOptionPane.showMessageDialog(null, "Debe seleccionar el estado del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                                                    }
                                                                }
                                                                else
                                                                {
                                                                    JOptionPane.showMessageDialog(null, "Debe seleccionar el plazo del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                                                }
                                                            }
                                                            else
                                                            {
                                                                JOptionPane.showMessageDialog(null, "Debe seleccionar el tipo de pago del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                                            }
                                                        }
                                                        else
                                                        {
                                                            JOptionPane.showMessageDialog(null, "Debe seleccionar el tipo de moneda del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                                        }
                                                    }
                                                    else
                                                    {
                                                        JOptionPane.showMessageDialog(null, "Debe digitar el correo del contacto del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                                    }
                                                }
                                                else
                                                {
                                                    JOptionPane.showMessageDialog(null, "Debe digitar el cargo del contacto del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                                }
                                            }
                                            else
                                            {
                                                JOptionPane.showMessageDialog(null, "Debe digitar el contacto del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                            }
                                        }
                                        else
                                        {
                                            JOptionPane.showMessageDialog(null, "Debe digitar la calle del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                        }
                                    }
                                    else
                                    {
                                        JOptionPane.showMessageDialog(null, "Debe digitar la ciudad del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                    }
                                }
                                else
                                {
                                    JOptionPane.showMessageDialog(null, "Debe digitar el país del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                                }
                            }
                            else
                            {
                                JOptionPane.showMessageDialog(null, "Debe digitar el limite de crédito del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                            }
                        }
                        else
                        {
                            JOptionPane.showMessageDialog(null, "Debe digitar el télefono del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                    else
                    {
                        JOptionPane.showMessageDialog(null, "Debe digitar el nombre del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                    }
                }
                else
                {
                    JOptionPane.showMessageDialog(null, "Debe digitar la cédula del proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
                }   
            }
            else
            {
                JOptionPane.showMessageDialog(null, "Debe digitar el código de proveedor", "Error en validación", JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if(opcion == 0)
            {
                ...
                if(FilaSelect >= 0)
                {
                    ...
                    {
                        str_Codigo = String.valueOf(Tabla.getValueAt(FilaSelect, 0));
                        rs = cls_Supplier.Obtener_id_Direccion();
                        while(rs.next())
                        {
    ...
    void cargar_datos()
    {
        try {
            con = new Conectar();
            ps = con.getConnection().prepareStatement("SELECT * FROM supplier");
            rs = ps.executeQuery();
            ...
            while(rs.next())
            {
                str_Codigo = rs.getString("supplier_id");
                str_Nombre = rs.getString("supplier_name");
                str_Estado = rs.getString("supplier_status");
                str_Usuario = rs.getString("user_name");
                date_FechaCreate = Formato.format(rs.getDate("supplier_create_date"));
                date_FechaModify = Formato.format(rs.getDate("supplier_modify_date"));
    ...
    private void Agregar()
    {
        try {
                ...
                int_Res = cls_Supplier.Insertar_Direccion(str_Pais ,str_Ciudad, str_Calle, str_CodigoPostal);
                if(int_Res > 0)
                {
                ...
                int_Res = cls_Supplier.Insertar_Proveedor(str_Codigo, int_Direccion, str_Cedula, str_Nombre, str_Telefono, str_Contacto, 
    ...
    private void Consultar(String str_Code)
    {
        try {
            ...
            rs = cls_Supplier.Buscar_Proveedor(str_Code);
            if(rs.next())
            {
                str_Codigo = rs.getString("supplier_id");
                str_Cedula = rs.getString("supplier_cod");
                int_Direccion = rs.getInt("address_id");
                str_Nombre = rs.getString("supplier_name");
                str_Telefono = rs.getString("supplier_phone");
                str_Contacto = rs.getString("supplier_contact");
                str_Correo = rs.getString("supplier_email_contact");
                str_Cargo = rs.getString("supplier_position_contact");
                str_SitioWeb = rs.getString("supplier_web");
                str_TipoMoneda = rs.getString("supplier_money_type");
                str_TipoPago = rs.getString("supplier_payment_type");
                int_Plazo = rs.getInt("supplier_credit_term");
                dbl_Credito = rs.getDouble("supplier_limit_credit");
                str_Estado = rs.getString("supplier_status");
                date_FechaCrear = rs.getDate("supplier_create_date");
                date_FechaModificar = rs.getDate("supplier_modify_date");
            ...
            rs = cls_Supplier.Buscar_Direccion(int_Direccion);
            if(rs.next())
            {
    ...
    private void Eliminar()
    {
         try {
             str_Codigo = this.txtCodigo.getText();
             int_Res = cls_Supplier.Borrar_Proveedor(str_Codigo);
             if(int_Res > 0)
             {
             ...
             int_Res = cls_Supplier.Borrar_Direccion(int_Direccion);
             if(int_Res > 0)
             {
    ...
    private void Modificar()
    {
         try {
                ...
                date_FechaModificar = new Date();
                DateFormat formato = new SimpleDateFormat("YYYY-MM-dd");
                int_Res = cls_Supplier.Editar_Direccion(str_Pais ,str_Ciudad, str_Calle, str_CodigoPostal, int_Direccion);
                if(int_Res > 0)
                {
                ...
                int_Res = cls_Supplier.Editar_Proveedor(int_Direccion, str_Cedula, str_Nombre, str_Telefono, str_Contacto, str_Cargo, 
